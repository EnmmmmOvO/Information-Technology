...
impl Hook {
    ...
    fn new<F: Fn(&mut Cpu) + 'static>(num_left: u32, callback: F) -> Self {
        Hook { num_left, callback: Rc::from(callback) }
    }

    // TODO: implement a call method
    fn call(mut self, cpu: &mut Cpu) -> Option<Hook> {
        if self.num_left == 0 {
            (self.callback)(cpu);
            None
        } else {
