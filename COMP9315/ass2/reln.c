...
int addTupleToPage(Reln r, Tuple t, Bits p)
{
...
PageID addToRelation(Reln r, Tuple t)
{
	...
	if (p == NO_PAGE) return p;

	r->ntups++;
	if (r->ntups == 0 || r->ntups % (1024 / (10 * nattrs(r))) != 0) return p;

	Page old_page = getPage(r->data, r->sp);
	Tuple tup = pageData(old_page);
	Offset ntups = pageNTuples(old_page);

	putPage(r->data, r->sp, newPage());
	addPage(r->data);

	while (TRUE) {
		for (int i = 0; i < ntups; i++, tup += strlen(tup) + 1) {
			addTupleToPage(r, tup, getLower(tupleHash(r, tup), r->depth + 1));
		}

		Offset next = pageOvflow(old_page);
		free(old_page);

		if (next == NO_PAGE) break;
		old_page = getPage(r->ovflow, next);
		tup = pageData(old_page);
		ntups = pageNTuples(old_page);
		putPage(r->ovflow, next, newPage());
	}

	r->sp = (r->sp + 1) % (1 << r->depth);
	r->npages++;
	if (r->sp == 0) r->depth++;
	return p;
}
