...
fn main() {
    ...
    let (ic, oc) = mpsc::channel::<(usize, usize)>();

    for (idx, chunk) in digits_operators.chunks(length / 6 + 1).enumerate() {
        let ic = ic.clone();
        std::thread::scope(|scope| {
            scope.spawn(move || {
                let mut count = 0;
                for (digits, operators) in chunk.into_iter() {
                    match calculate(digits.to_vec(), operators.to_vec()) {
                        Ok(true) => count += 1,
                        _ => {},
                    }
                }
                ic.send((idx, count)).unwrap();
            }).join().unwrap();
        });
    }
    ...
    for _ in 0..6 {
        let (idx, num) = oc.recv().unwrap();
        total += num;
        println!("Thread {idx} found {num} combinations");
    }
    println!("Total: {total}");
}
