...
fn search_pokedex<'a>(query: &str, pokedex: &'a Pokedex, search_results: &mut Vec<FoundPokemon<'a>>) {
    for pokemon in pokedex {
        ...
        if contains_en || contains_cn || contains_jp {
            match search_results.iter_mut().find(|found_pokemon| found_pokemon.pokemon == pokemon) {
                ...
                None => search_results.push(FoundPokemon {pokemon, matching_queries: vec![query.to_string()]})
            }
        }
    }
}
